...
from rlgym_ppo.ppo.a2c_learner import A2CLearner


class A2C_Learner(object):
    def __init__(
            # fmt: off
            self,
            env_create_function,
        ...
        # Initialize A2C or PPO based on the flag
        if use_a2c:
            print("Initializing A2C...")
        ...
        self.agent.policy = self.learner.policy
    ...
    def update_learning_rate(self, new_policy_lr=None, new_critic_lr=None):
        if new_policy_lr is not None:
            ...
            for param_group in self.learner.policy_optimizer.param_groups:
        ...
        if new_critic_lr is not None:
            ...
            for param_group in self.learner.value_optimizer.param_groups:
    ...
    def _learn(self):
        ...
        while self.agent.cumulative_timesteps < self.timestep_limit:
            ...
            # Use the learner to compute updates
            report = self.learner.learn(self.experience_buffer)
    ...
    def add_new_experience(self, experience):
        ...
        value_net = self.learner.value_net
    ...
    def save(self, cumulative_timesteps):
        ...
        # Save the learner's state
        self.learner.save_to(folder_path)
    ...
    def load(self, folder_path, load_wandb, new_policy_lr=None, new_critic_lr=None):
        ...
        self.learner.load_from(folder_path)
        ...
        with open(os.path.join(folder_path, "BOOK_KEEPING_VARS.json"), "r") as f:
            ...
            self.learner.cumulative_model_updates = book_keeping_vars[
